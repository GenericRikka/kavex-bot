...
class MinecraftLink(commands.Cog):
    ...
    @group.command(name="connect", description="Link a channel or thread to a Minecraft server using its token")
    @app_commands.default_permissions(administrator=True)
    @app_commands.describe(token="Paste token from plugins/KavexLink/secret.txt",
    ...
    async def connect(self,
        ...
        if isinstance(channel, discord.TextChannel):
            ...
            if isinstance(channel.parent, discord.TextChannel):
                ...
                await interaction.response.send_message(
                    "Unsupported thread parent type for webhook posting.", ephemeral=True
                )
                return
        else:
            await interaction.response.send_message(
                "Unsupported channel type. Use a text channel or a thread.", ephemeral=True
            )
            return
        ...
        if row and row["webhook_url"]:
            ...
            try:
                ...
                await db.conn.execute(
                    "INSERT OR REPLACE INTO mc_webhooks(guild_id, channel_id, webhook_url, thread_id) VALUES (?,?,?,NULL)",
                    (interaction.guild_id, parent_channel.id, webhook_url),
                )
                await db.conn.commit()
            except discord.Forbidden:
                await interaction.response.send_message(
                    f"❌ I need **Manage Webhooks** in {parent_channel.mention}.", ephemeral=True
                )
                return
            except Exception as e:
                await interaction.response.send_message(
                    f"❌ Failed creating webhook in {parent_channel.mention}: `{e}`", ephemeral=True
                )
                return

        # Store the link (the linked channel may be a thread or a text channel)
        th = token_hash(tok)
        await db.conn.execute(
    ...
    @app_commands.default_permissions(administrator=True)
    async def disconnect(self, interaction: discord.Interaction, channel: typing.Union[discord.TextChannel, discord.Thread]):
        await db.ensure_connected()
        await db.conn.execute(
    ...
    @group.command(name="status", description="Show link status for a channel/thread")
    @app_commands.default_permissions(administrator=True)
    async def status(self, interaction: discord.Interaction, channel: typing.Union[discord.TextChannel, discord.Thread]):
        await db.ensure_connected()
        cur = await db.conn.execute(
    ...
    async def debug_links(self, interaction: discord.Interaction):
        await db.ensure_connected()
        cur = await db.conn.execute(
            "SELECT channel_id, server_name, status FROM mc_links WHERE guild_id=?",
    ...
    @group.command(name="test_send", description="(Admin) Try sending a test line to a channel/thread (via stored webhook)")
    @app_commands.default_permissions(administrator=True)
    async def test_send(self, interaction: discord.Interaction, channel: typing.Union[discord.TextChannel, discord.Thread]):
        # resolve parent webhook + thread_id like mc_ws does
        await self._ensure_webhook_schema()
        parent_id = channel.id if isinstance(channel, discord.TextChannel) else channel.parent.id  # type: ignore
        thread_id = None if isinstance(channel, discord.TextChannel) else channel.id  # type: ignore
        ...
        if not row or not row["webhook_url"]:
            await interaction.response.send_message(
                "No webhook stored for this channel parent. Re-run /minecraft connect.", ephemeral=True
            )
            return

        # Send via discord.py (we're in interaction context, safe)
        try:
            wh = await channel.guild.fetch_webhook(int(row["webhook_url"].split("/")[-1]))  # best-effort; optional
        except Exception:
            wh = None
        try:
            if wh:
                await wh.send("KavexLink: test message", username="KavexLink")
            else:
                # Fallback: simple channel send
                await channel.send("KavexLink: test message")
            await interaction.response.send_message(f"Sent test to {channel.mention}.", ephemeral=True)
        except Exception as e:
            await interaction.response.send_message(f"❌ Send failed: `{e}`", ephemeral=True)
